...
import com.example.mallet.frontend.view.common.activity.ActivityOpening;
...
public class FragmentProfile extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        String credential = CredentialsHandler.get(getActivity());
    ...
    private void setupContents() {
        ...
        ViewUtils.hideItems(themeRg);
        ViewUtils.disableItems(themeRg);

        themeLl.setOnClickListener(v -> {
            themeClickCounter[0]++;
            if (themeClickCounter[0] % 2 != 0) {
                ViewUtils.showItems(themeRg);
                ViewUtils.enableItems(themeRg);
            } else {
                ViewUtils.hideItems(themeRg);
                ViewUtils.disableItems(themeRg);
            }
    ...
    private void logOut() {
        clickCount++;
        if (clickCount == 1) {
            ViewUtils.showToast(getContext(), "Click once again to log out");
    ...
    private void deleteAccountDialogWithRestart(int attemptCount) {
        Dialog dialog = ViewUtils.createDialog(requireContext(), R.layout.dialog_delete_account, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT), Gravity.BOTTOM);
        ...
        deleteAccConfirmTv.setOnClickListener(v -> {
            if (!deleteAccCb.isChecked()) {
                ViewUtils.showItems(deleteAccCbErrTv);
            } else {
                ViewUtils.hideItems(deleteAccCbErrTv);
                ViewUtils.enableItems(deleteAccConfirmTv);
                ...
                userService.deleteUser(userId, new Callback<Void>() {
                    ...
                    public void onFailure(Call<Void> call, Throwable t) {
                        if (attemptCount < MAX_RETRY_ATTEMPTS) {
                            ...
                            ViewUtils.showToast(requireContext(), "Network error");
                            ViewUtils.hideItems(progressBar);
                        }
                    }
                });
