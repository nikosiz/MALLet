package com.example.mallet.frontend.view.common.fragment;
...
import com.example.mallet.frontend.view.common.activity.ActivityLearn;
...
public class FragmentLearn extends Fragment {
    ...
    private void learnOptionsDialog() {
        Dialog dialog = ViewUtils.createDialog(requireContext(), R.layout.dialog_learn_options, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT), Gravity.BOTTOM);
    ...
    private void handleNextTvClick() {
        ...
        if (isCorrect) {
            ...
            if (currentQuestionIndex < MAX_QUESTIONS && currentQuestionIndex < writtenQuestions.size()) {
                ...
                ViewUtils.showItems(finishTv);
                ViewUtils.enableItems(finishTv);
                ViewUtils.hideItems(nextTv);
                ViewUtils.disableItems(nextTv);
                learningFinishedDialog();
            }
        } else {

            ViewUtils.showItems(answersLl);
        }
    ...
    private void learningFinishedDialog() {
        Dialog finishedDialog = ViewUtils.createDialog(requireContext(), R.layout.dialog_learn_finished, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT), Gravity.BOTTOM);
        ...
        dialogRestartTv.setOnClickListener(v -> {
            currentQuestionIndex = 0;
            writtenQuestions = activityLearn.generateWrittenQuestions();
            ViewUtils.hideItems(finishTv);
            ViewUtils.disableItems(finishTv);
            ViewUtils.showItems(nextTv);
            ViewUtils.enableItems(nextTv);
            finishedDialog.dismiss();
            learnOptionsDialog();
        });
